  ...
  <div style="width: 100%" class="user-manage">
    ...
    <el-table :data="tableData" style="width: 100%" row-style="height:50px" class="table-data" v-loading="loading">
      <el-table-column fixed  label="文章名" width="250" show-overflow-tooltip >
        ...
        <div class="table-row-title" @click="titleClick(scope.row)">{{ scope.row.title }}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="审核操作" width="150">
        <template #default="scope">
          <div v-if="scope.row.status === 'release'">
            <el-tooltip content="审核成功" placement="top">
            <el-icon class="check-icon" @click="btnClick(scope.row,true)">
              <Check/>
            </el-icon>
          </el-tooltip>
          <el-tooltip content="审核失败" placement="top">
            <el-icon class="close-icon" @click="btnClick(scope.row,false)">
              <Close/>
            </el-icon>
          </el-tooltip>
          </div>
...
import { getSortList, getSortDetail, getModifySort, getAddSort, getPagePostList, getAllSorts,getCheckPost} from '@/api/mode1';
...
const btnClick = async (item:any, isCheck: boolean) => {
